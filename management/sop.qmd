...
In regular [planning and review sessions](/management/planning_review_sessions.html),
we reflect on our progress, define objectives, and identify key results that align with our broader mission.
...
For example, there should always be a specific repository containing the current version of a research paper,
along with the version history, ongoing work in branches or pull requests and open issues.
We try to avoid sending direct messages and documents as much as possible.
Collaborating in these repositories has many benefits: repositories provide context (clearly identifying the project, its history, ongoing work, and open issues), 
llow us to reduce direct messages (the cognitive effort to search for related documents and coordinate asynchronous work), and use scripts like Labot for automation.
...
- **Specific element in our handbook:**  
  </research/revision.html#expected-outcome>

- **Selected slide in our project:**  
  <https://fs-ise.github.io/open-source-project/output/06-best_practice.html#technical-setup>

- **A comment in an issue discussion:**  
  <https://github.com/fs-ise/digital-work-lecture/issues/8#issuecomment-2199793685>

#### How to create a deep link

- Click on the anchor and copy the URL:

  ![Deep-link example handbook](/assets/images/deep-link-handbook.gif)

- In slides, add the title after the `#`:

  ![Deep-link example slides](/assets/images/deep-link-slides.gif)
...
See Ben Balter’s [post](https://ben.balter.com/2015/11/18/tools-to-empower-open-collaboration/){target=_blank}.

### Sharing contents and research results

- **Work iteratively:** Always write and share bullet points before writing paragraphs.
- **Agree on short names for research projects:** For example, use names like *BibDedupe* or *GitIntro*.
  Short names should be related to the topic, not the target journal (avoid referring to “the MISQ paper”).
...
- Questions related to our processes should be answered by sharing a link instead of answering the same question multiple times (e.g., [here](/research/writing_technical.html#citations-and-reference-sections) are our guidelines for citing in Markdown).
