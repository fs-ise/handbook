...
It reduces friction and helps us focus on the work that matters most.
The following documents [how to suggest changes](#how-to-suggest-changes) and outlines the underlying [principles](#principles).
...
- **Actionable first**: Start with the actionable items first and present rationales and details at the end of the page.
...
::: {.callout-important}
Each sentence should be on a new line (to ensure that the git diff is readable).
To create a paragraph, add an empty line.
...
{{< iconify hugeicons:add-circle title="Issues" label="Issues" >}}
...
Icons: [hugeicons](https://icon-sets.iconify.design/hugeicons/){target=_blank} preferred.
...
::: {.callout-note title="Mermaid diagrams" collapse=true}
Use Quarto's default Mermaid rendering for visible diagrams.
Do not set `mermaid-format: svg` globally in `_quarto.yml`,
because global SVG rendering can add fixed heights
and excessive whitespace to shallow diagrams.

Use pre-generated SVG only for Mermaid diagrams inside collapsed callouts.
JavaScript rendering can calculate incorrect dimensions
when the containing callout is hidden during page load.
Add the cell option immediately after the Mermaid fence.

````markdown
```{mermaid}
flowchart LR
  A[Visible diagram] --> B[Default rendering]
```
````
...
%%| mermaid-format: svg
flowchart LR
  A[Hidden at load] --> B[Pre-generated SVG]
```
:::
````
:::
...
Key principles of the handbook include:
...
For anything that must be correct and consistent over time (e.g., **course offerings**, **teaching dates**, **presentations**, **research projects**, **paper lists**, **lab processes**),
the handbook should be the **single source of truth**.
...
[{{< iconify hugeicons:database-expand style="color: white;" >}}  Data directory](https://github.com/fs-ise/handbook/tree/main/data){.btn .btn-primary .btn-center rel="noopener" target=_blank}
...
Note: ideally, reports should be updated in the /reports directory and included through scripts (python or markdown-includes)
...
- [Hyndman](https://robjhyndman.com/){target=_blank}, [Heiss](https://www.andrewheiss.com/){target=_blank}, [Canouil](https://mickael.canouil.fr/){target=_blank}

See @Rhymer2023 on document-mediated, open, and asynchronous organizing practices.