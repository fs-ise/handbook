...
The repository conformance checks run separately from Quarto rendering.
Quarto only includes the previously generated static report fragment below, so rendering the handbook does not clone repositories or call the GitHub API.
...
The scheduled and manually triggered GitHub Actions workflow is defined in [update_repository_conformance.yml](https://github.com/fs-ise/handbook/actions/workflows/update_repository_conformance.yml){target=_blank}.
For branch-protection checks across repositories, the workflow can use `LABOT_PAT_TOKEN` when the default `GITHUB_TOKEN` is not sufficient.

{{< include _generated/repository-conformance.qmd >}}
